...
public class GmailService implements EmailService {
    ...
    public void validationLink(Account acct) {
        try {
            String link = String.format( // 驗證信件連結verify.java > verify.jsp
                    "http://localhost:8080/Accounting/verify?email=%s&token=%s", acct.getEmail(), acct.getPassword());

            String anchor = String.format("<a href='%s'>驗證郵件</a>", link);

            String html = String.format("請按 %s 啟用帳戶或複製鏈結至網址列：<br><br> %s", anchor, link);

            javax.mail.Message message = createMessage( // 寄信
                    mailUser, acct.getEmail(), "ACCOUNTING 註冊結果", html);
    ...
    public void failedRegistration(String acctName, String acctEmail) {
        try {
            javax.mail.Message message = createMessage(mailUser, acctEmail, "ACCOUNTING 註冊結果",
    ...
    public void passwordResetLink(Account acct) {
        try {
            String link = String.format("http://localhost:8080/Accounting/reset_password?name=%s&email=%s&token=%s",
            ...
            javax.mail.Message message = createMessage(mailUser, acct.getEmail(), "ACCOUNTING 重設密碼", html);
            Transport.send(message);
        } catch (MessagingException | IOException e) {
            throw new RuntimeException(e);
        }
